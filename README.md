...
| `deck(margin:)`                 | How far the layout is held off the edge, default 60pt. It is the page's own margin, so the PDF's margin and every frame's inset are the same figure, written once; `slide` reads it back.                                                                                                                                                                                                        |
| `player(…)`                     | vit's half of a deck on its own: the stylesheet, the runtime and the chrome, around pages that are already laid out. `deck` is a document — page, text, title — and then this. A package that already *is* the document sets its page as it always did and calls `player` with the settings above that are not the document's: `pdf`, `duration`, `easing`, `transition`, `theme`, `laser`. |
...
Every event is `vit:` and a word for what has just become true; the three
moments of a move share the `move-` stem. On every move the deck dispatches all
three on `.vit-deck`, one for one:

| event | when |
| --- | --- |
| `vit:move-begin` | a move has been accepted and nothing has been captured yet |
| `vit:move-here` | the deck is at the new position — it is in the DOM |
| `vit:move-done` | the move has finished moving |

`begin` carries `detail: { index, step, mode }` and says where the deck is
*going*: nothing has been drawn, so `vit.index` is still the frame being left,
and a move cut short by the next is a `begin` whose destination never arrived.
It is there so that another window showing this deck can start its own move
alongside rather than behind. `here` and `done` carry `detail: { index, step }`
and say where the deck *is* — at `here` the position is in the DOM but not yet
on the screen, since what a page draws waits for the transition to end. A move
with nothing to animate settles at once, one cut short by the next settles
where it stopped, and opening the overview is a transition but not a move, so
it announces none of the three.

The deck's own two are `vit:drawn`, whenever it has drawn itself — a move or a
