...
curve at the next phase, a pendulum at the next pose) is interpolated on its SVG
nodes by the Web Animations API. There is no interpolation code of our own: the
runtime navigates, tidies the DOM once at load, and hands the browser two
states.
...
One compile per format, from the same file. Open `examples/tutorial.html` in a
...
`examples/tutorial.typ` is the tutorial deck: twenty-six pages, each one the
code on the left and what it does on the right, going through every call in the
API and saying when to reach for it.

Requires Typst 0.15 (`html`) and a browser with same-document View Transitions
including transition types and `view-transition-class`, and CSS `d` for path
morphing — Blink and Gecko have it; in Safari (27 preview) switch on the "CSS d
property" feature flag under Develop → Feature Flags. The deck is verified in
Chromium (`tools/`).
...
around the thing that should morph. Turning the page, the title shrinks from the
centre into the corner. Its geometry is _measured_ from the layout at load time,
never written by hand (`tools/check.mjs` prints it, as percentages of the page):
...
`deck` is the document: `#show: deck.with(title: "My talk")` at the top and the
slides below is the same as passing the content in.

The full API reference is generated by
...
Frames still transition with View Transitions, so "the second frame has one more
line" is an element-level interpolation, not a page jump — what `#pause` is
meant to look like, without parsing the content. In the PDF each frame is a
...
Several bodies in one `mark` are the states of one object. `→` moves to the next
state, `←` back along the same path, and the page turns only once the states are
exhausted.
...
function `f(t)` fed a few values of `t`. Everything that can be interpolated is,
and the runtime rewrites what the browser would refuse so that it can:
...
- `follow: "track"` runs the mark's centre along the first `<path>` of another
  ...
  tangent.
- Neither, and the `mark` of that key carries several states: the states *are*
  the keyframes, played continuously, one animation per node, read exactly like
  a step of an element animation. The tutorial's clef — komet's FFT, run at
  compile time, turns a treble clef into 45 rotating vectors and 60 states of the
  chain are played with `anim: (clef: (duration: 6000))` — and its sea, 25 states
  of three stacked sines, are these. A mark named this way counts no steps, and
  the PDF shows its first state.

Two rules follow from what the browser does between two states, and the clef is
built around both. It moves every vertex in a straight line at a constant speed,
so anything written *relative to something that moves* — a trail listed behind a
pen — has all its vertices change velocity at every keyframe, and the figure
judders at the state rate. Say it instead as fixed geometry whose paint changes,
or as a `stroke-dasharray` on one fixed path (four numbers, and they
interpolate), and there is no geometry to interpolate wrongly. And the states
are a sampling rate: what turns more than half a turn between two of them cannot
be recovered, which is what stops the clef's series at 45 terms.

Make the first state and the last equal and the loop has no seam; one turn of
the clef's series is one period, so it needs no `alternate`.

Animations start once the page transition has finished, pause when the frame is
left or the overview opens, and do not play under `prefers-reduced-motion`.
...
shadows vit' `mark` (`expected content, found array`); import by name. Inside a
canvas block, `circle` is CeTZ's as well — Typst's is `std.circle`.
...
The toolbar in the bottom-right corner (counter, desk, overview, laser, speaker
view, PDF, full screen) hides after 2.4 s without mouse movement — on the desk
it stays. The PDF button is a real `<a download target="_blank">`, so it never
navigates the deck away mid-presentation.
...
`window.vit.mode = "present"` opens straight into the presentation. The rail is
the deck itself in the overview's thumbnails, and the preview is a copy of this
same HTML at a `#page.position` hash — the same mirror the speaker view uses, so
there is one renderer and the preview plays the real transitions.
...
bigger, put it in a layer of its own, and the layers already on the page glide
to their new place as whole pictures. A layer that has to reach into an earlier
one (an arrow into a diagram it does not draw) draws that one's anchors hidden,
so the package still lays them out in the same place.

The tutorial's assembly page is the two together, and it is the whole of that
page: one diagram where `A`, `p` and `q` arrive at step 2, and one layer with
`X` and its arrows arriving at step 3 — the square glides over as it lands.
...
from it zooms that page into place; turning pages morphs elements. Those are two
different transitions, and the overview uses the first.
...
dot on the main window. It works over `file://` and closes with the main window.
...
`tools/` need `playwright` (`paths.mjs` only Node); `verify.mjs` also needs
`pdftoppm` (poppler) and `compare` (ImageMagick). To inspect a transition, take
over `document.startViewTransition`, `await vt.ready`, then pause and seek every
...
vit is, as far as we know, the first to put Typst and the View Transitions API
together.
