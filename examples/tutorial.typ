...
// Until then it is the package as built (`npm run build`), not typst/.
#import "../dist/0.1.0/lib.typ": *
// Element animation, stepped or played, comes from tween, not from the deck.
...
#let body = {
  ...
  slide(
    ...
    note: [
      ...
      →: the title shrinks into the corner and stays there for the rest of the deck. One `mark` is one object.
    ],
  )[
  ...
  let hello(k) = screen(if k == 1 [
    ...
    #text(size: 21pt, fill: dim)[
      The rest of the page arrives around it. Two ordinary Typst
  ...
  #deck(title: "My talk")[
    ...
    #slide[
      ...
      The rest of the page arrives around it.
    ]
  ]
  ```)
  slide(
  ...
  let item(n, hue, what) = block(
    ...
    stroke: (left: 3pt + hue),
    text(size: 21pt, fill: hi)[*#n*#h(0.6em)#what],
  )
  slide(
  ...
  slide(
    ...
    note: [D arrives and nothing else moves, because D's space was already there. Identical layout on every frame is what lets the marks morph instead of the page reflowing under them. `build` stacks, `reveal` holds still.],
    ..reveal(3, (step, at) => lesson(
      ...
      when: [What has not arrived is `hide`n and holds its box; anything else is `none`, which switches a stroke off.],
    )),
  )
  ...
  slide(
    ...
    transition: "slide",
    note: [This page came in with `slide`: press ← and → to watch it again. These nine names are all the effects; everything else is a setting. Between frames of one page the root always cross-fades and elements enter by `mark(transition:)` instead.],
  )[
    #lesson(
      "Page transitions",
  ...
  let ring = cetz.canvas(length: SU, {
    ...
    circle(
      ...
      // and that number is the only thing over() varies, so the ring itself is
  ...
  slide(
    ...
    #lesson(
      "Continuous animation",
      [`waapi.animate` is Web Animations from Typst: keyframes and options as the API takes them, handed to `el.animate()` as they are. There is no DSL of our own and no deck required, so it moves the same figure in a blog post.],
      src(```typ
      #let ball(i) = (
        keyframes: (
  ...
  // A treble clef, outlined the way a handwriting tutorial draws one. It is not
  // the boundary of the filled glyph, which is four separate curves (the
  // silhouette and the three holes the stroke encloses), but the outline of the
  // *pen*: the
  ...
  // One piece per state: a piece spans exactly
  ...
  // keyframe in which the pen crosses it, so the ink ends on the pen at every
  ...
  slide(
    ...
    note: [→ turns every arrow round. Both states are the same drawing, so the arrowheads slide along the arrows instead of fading. This is an element animation on someone else's diagram.],
  )[
    #lesson(
  ...
  slide(
    ...
    ..(
      ...
      "Inside a formula",
      [A mark is content and a formula is made of content, so a term is marked where it stands, passed *as an equation*, not as bare math.],
      src(```typ
      #let sq = mark.with("sq")
      #let rhs = mark.with("rhs")
      #let half = mark.with("half", transition: "rise")
