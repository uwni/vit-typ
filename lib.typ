...
#let _enter-side(x) = (
  _type("fwd", "enter-" + x) + "::view-transition-new(root)",
  _type("back", "enter-" + x) + "::view-transition-old(root)",
) + _enter-mark(x)

/// The side the pair's leave effect governs: the old side forward, the new side back.
#let _leave-side(x) = (
  _type("fwd", "leave-" + x) + "::view-transition-old(root)",
  _type("back", "leave-" + x) + "::view-transition-new(root)",
) + _leave-mark(x)
...
#let _knobs = {
  let k = (:)
  for m in (read("deck.css") + _effects).matches(regex("var\(\s*--vit-([a-z0-9-]+)")) { k.insert(m.captures.first(), true) }
  k
}
