/* The invariants: properties every build must have, whatever the deck says.
   Each check tests a property rather than a symptom, because every defect this
   suite catches first showed up as one of these properties failing somewhere
   nobody was looking.
...
const results = [];
/* `ok` may also be "n/a": a property this deck has nothing to test it with. It
   is reported as such instead of counting as a pass, since a check that never
   ran did not hold. */
...
const player = ["web/runtime.js", "web/chrome.js"].map(f => readFileSync(join(ROOT, f), "utf8")).join("\n");
const lib = readFileSync(join(ROOT, "typst/lib.typ"), "utf8");
...
try {
  ...
  /* A thumbnail is exactly one picture, wherever the rail is up and whatever a
     hovered dot is previewing, and previewing must not move the thumbnails
     ...
     still be clicked. */
  {
      ...
      const turn = async mode => {
        ...
        const out = {
          ...
          /* A name is a capture and a capture is a hole, so under boxed nothing
             outside the page may carry one, especially the toolbar and the
             laser, since they sit over what the presenter is looking at. */
          outside: (() => {
    ...
    /* The price of that: for a frame at the start of a transition the deck is
       ...
       (the same <use> as a thumbnail, at the deck's own size), or the desk
       shows through the middle of the screen. */
  ...
  /* While the deck moves, the browser cannot say where the pointer is: what was
     ...
     and arriving on <html> although it has not moved. If that were trusted, the
     toolbar would hide for every transition and come back after it. */
  {
    const r = await page.evaluate(`
  ...
  /* A move is said out twice: once when it has been accepted and nothing has
     ...
     window showing this deck start at the same time, so it has to be sent
     early, before the capture, and it has to carry where the
  ...
  /* The clock times the talk, not the window. It cannot be checked here, since
     this deck has been on stage since the model was read, so it is checked in a
  ...
  /* A preview inside the speaker view is this same document in a window that
     ...
     reached into from here. It is checked in a window of its own instead,
    ...
    const ask = async (name, port) => {
      ...
      try {
        ...
        return await w.evaluate(`
          ...
          /* The pace is sent to it, and only by the window that made it. Here
             there is no such window (this one was opened by the suite, not by
             a deck), so what can be checked is the half that must hold anyway:
  ...
  /* Everything the player can be asked to do, once each. The captures are
     counted across all of it. */
  {
    const vit = await page.evaluate(`
      ...
      /* And once more with nothing watching: a rejection the player does not
         ...
         that a transition is overtaken before it is even ready. Any later and
    ...
    /* A transition overtaken by the next one is skipped on purpose. That is
       how the player handles a presenter pressing faster than the deck moves.
...
{
  const bare = html.replace(/<script id="vit-chrome">[\s\S]*?<\/script>/, "");
