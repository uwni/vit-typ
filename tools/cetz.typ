// The CeTZ layer's whole claim, as a document: a canvas whose elements carry
// states draws exactly what the same canvas draws without them.
//
// Every case is two pages: the plain CeTZ one first, the other second. The
// test is that the pair is *pixel-identical*, with no tolerance. That is
// what says the wrapping is only about the states and never about geometry: the
...
//
//   typst compile --root . tools/cetz.typ /tmp/t.pdf
...
#import "../packages/tween/compat/cetz.typ": tweened

#let cz = tweened(cetz)
#set page(width: 340pt, height: 240pt, margin: 10pt, fill: white)

// ── 1. an element with states among plain ones ───────────────────────
// its name is still its own, its anchors still resolve from outside, and the
// current point after it is where it left it. One state: the wrapping alone,
// with nothing about the picture changed, is what this case is about
#let one(k) = cetz.canvas(length: 1cm, {
  import cetz.draw: *
  let (circle: kcircle, content: kcontent, over) = cz
  rect((-1, -1), (5, 3), stroke: none)
  line((-0.5, -0.5), (4.5, 2.5), stroke: 2pt + blue)
  if k {
    kcircle((1, 1), radius: over(0.5), fill: red, stroke: 1pt + black, name: "c")
    kcontent((1, 1), over(text(fill: white)[x]))
  } else {
...
#let two(k) = cetz.canvas(length: 1cm, {
  ...
  scale(1.1)
  rect((-1, -1), (5, 3), stroke: none)
  if k { kcircle((1, 1), radius: over(0.5), fill: red) } else { circle((1, 1), radius: 0.5, fill: red) }
...
#let three(k) = cetz.canvas(length: 1cm, {
  ...
  rect((-1, -1), (5, 3), stroke: none)
  if k {
    kcircle((over(1, 2, 3.5), 1), radius: over(0.3, 0.5, 0.8), fill: red)
  } else {
...
// intersections with a circle that has states, one inside a plain group (whose
// bounds must not move), and a group that has them itself
#let four(k) = cetz.canvas(length: 1cm, {
  import cetz.draw: *
  let (circle: kcircle, rect: krect, group: kgroup, over) = cz
  rect((-1, -1), (6, 3), stroke: none)
  intersections("i", {
    if k {
      kcircle((1.5, 1), radius: over(0.8), stroke: 2pt + blue, name: "c")
    } else {
  ...
  group(name: "g", {
    if k { krect((3, 0), (4.2, 1.2), fill: over(aqua)) } else { rect((3, 0), (4.2, 1.2), fill: aqua) }
  ...
  if k { kgroup(name: "h", over(body)) } else { group(name: "h", body) }
...
// placement has to be deaf to them, or the element drifts, gets a border, or is
// scaled twice
#let six(k) = cetz.canvas(length: 1cm, {
  ...
  rect((-1, -1), (5, 3), stroke: none)
  if k {
    kcircle((1.5, 1), radius: over(0.6), fill: red)
  } else {
