/// Settings the presenter chooses: how fast the deck moves and what the laser
/// looks like. Every control names what it sets in `data-set`, and the runtime
/// keys on that alone, so a new dial is a row here and an entry there. The values live in the browser, so a deck handed to someone else
