...
(() => {
  ...
  /* black screen (b / .): everything in the body is hidden so the audience looks at the presenter; the keys still work */
  let black = false;
  const toggleBlack = () => {
    black = !black;
  ...
  /* the controls show what is in force, whatever changed it: a key, a panel in
     another window, or the deck itself */
  const syncSettings = () => {
  ...
  /* ── when the toolbar is out ──────────────────────────────────────────
     ...
     browser's answer. While the deck moves nothing is hit-tested, so there is
     no new answer and the last one stands. */
  let bar = null;
  ...
  /* ── the tracer ──────────────────────────────────────────────────────
     ...
     than the document left room for (the ceiling below), and it takes at most
     one point a frame, or a 1000 Hz mouse would spend the whole tracer inside a
     few milliseconds. */
  const FRAME = 16;
  ...
  /* All four are the document's: deck(laser:) writes them into the stylesheet
     and findLaserLook() reads them back. There are no defaults here, because a
     second copy of a value would be a second thing to change. */
  let laserUrl = "", laserInk = "", laserSize = 0, laserPx = 0, laserTrail = 0;
  ...
  const drawTrail = () => {
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      /* The tracer is the dot's own streak, so it is drawn to the dot's size,
         ...
         dropping off, is the whole fade. */
  ...
  /* The mouse's dot is the cursor, so there is nothing to place, but the
     tracer is drawn here whichever pointer is in use. */
  ...
  /* Which pointer is in use, and where it is. Both are the event's own, and both
     survive a transition. A transition takes away a pointer's *target*, not
     its position. */
  const route = e => {
  ...
  const openSpeaker = () => {
    ...
    /* the window's whole body, its toolbar included, and the same two dialogs:
       the elements themselves, cloned, so there is no second copy of the markup
       to keep in step */
    ...
    /* A key pressed here is a key pressed at the deck, so this window doubles
       as a remote. The exception is the chrome's own keys, which act on the
       window they were pressed in: the help and the settings belong where the
  ...
  /* ── keys ─────────────────────────────────────────────────────────────
     ...
     Keys are caught on the way down, so it does not matter who bound a listener
     first. */
  const KEYS = {
    f: toggleFullscreen,
    l: toggleLaser,
  ...
  /* ── start ────────────────────────────────────────────────────────────
     The deck says when it is live. Either preview inside the speaker view is
     driven by the window that opened it, so it skips all of this. */
  ...
  const start = () => {
    ...
    if (window.name.startsWith("vit-mirror")) {
      ...
      /* A preview shows what the audience is shown, so it needs the pace the
         presenter set, since speed divides a transition and a drawing alike.
         The pace is sent to it rather than looked up: over file:// every other window is another
