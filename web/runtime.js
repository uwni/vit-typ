/* ── vit · runtime ─────────────────────────────────────────────────
   ...
   · Stepping: the arrow keys walk the states of the drawings on a frame,
     which tween interpolates. Animations that play by themselves are only
     paused by the deck.
   This file is only the deck. What floats over it (the toolbar, the laser
...
(() => {
  ...
  const buildModel = () => {
    ...
    for (const el of deck.querySelectorAll(".vit-group")) {
      /* One page is a group in the deck and a thumbnail in the rail, in the
         same order, because the deck emitted both from one list of pages. */
  ...
  /* Everything the state is, on the screen, and nothing else: which frame is
     ...
     nothing and decides nothing, so running it twice is the same as running
     it once.

     A hovered dot is the one exception: a preview changes only what
  ...
  /* Report where we are to the address bar and to any listener. This runs on
     every page change and every step, but not on a change of mode, which is a
  ...
  const announce = (scroll = false) => {
    ...
    /* A followed path is a string of pixels, so it is measured again whenever
       the layout has moved. Only a frame that is on screen has a box to
  ...
  const syncRail = (scroll = false) => {
    ...
    groups.forEach((g, k) => {
      ...
      /* Dots show progress, not position: everything passed is solid, the current
         one a notch brighter. The same rule applies to every page: pages behind
         us are fully solid, pages ahead fully hollow. */
  ...
  const readDots = () => {
    for (const g of groups) {
      ...
      g.dots.forEach((el, d) => {
        ...
        if (!q) return;
        el.vitPos = q;
        /* hovering swaps the thumbnail to that step, so there is no need to open
           the page to see which step is which */
  ...
  /* A name each, and whatever its object declared: the pair of effects and the
     ...
     that morphs carries them too. Its effects then match no animation rule
  ...
  /* A step's animations are the deck's to cancel when the next one starts; a
     ...
     (they are looked up by role). What it cannot keep is kept here: the
  ...
  /* ── continuous animation ────────────────────────────────────────────
     ...
     and waapi start it. What is left here is when it may run: a drawing moves
     only while its frame is the one on stage, the
  ...
  /* ── the one way the deck changes ─────────────────────────────────────
     ...
     left out meaning "as we are". What the change animates depends only on
  ...
  const move = (where, done) => {
    ...
    /* Dispatched before anything is captured, so another window showing this
       deck can begin its move at the same time. It carries where the deck is
       going, since nothing has been drawn yet. One for
  ...
  const land = () => {
    ...
    /* The deck, for anything that shows it or drives it, including the
       player's own chrome, which is written against this and nothing else. */
    window.vit = {
      go, next, prev,
      ...
      /* Something else (the laser, for example) is using the pointer, so a
         press on the page is not a page turn. Touch only: a mouse can point and click. */
      hold: on => { held = !!on; },
